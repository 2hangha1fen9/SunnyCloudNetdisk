...
namespace WebDisk_Admin.DAL
{
    ...
    class UserDao
    {
        ...
        {          
            try
            {
                string sql = "SELECT User,Password,Mail,if(status = '1','启用','禁用')as status,Dir,if(ULBandwidth = 0,'无限制',ULBandwidth) as ULBandwidth,if(DLBandwidth = 0,'无限制',DLBandwidth) as DLBandwidth,if(QuotaSize = 0,'无限制',QuotaSize) as QuotaSize,if(QuotaFiles = 0,'无限制',QuotaFiles) as QuotaFiles from ftpd";
