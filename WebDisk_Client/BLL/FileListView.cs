...
namespace WebDisk_Client.BLL
{
    ...
    public class FileListView
    {
        ...
        /// </summary>
        /// <param name="path">当前路径</param>
        /// <param name="describe">文件夹名</param>
        ...
        public void RenameFile(LinkedList<string> path, FileDescribe describe, string newName)
        {
            try
            {
                ...
                if (share != null)
                {
                    share.AbsPath = $"{Settings.Default.absRootPath}/{share.UserName}/{p}/{newName}";
                    share.FileDescribe.Name = newName;
        ...
        public void RenameFile(string path, string newName)
        {
            try
            {
                ...
                ShareDescirbe share = manager.GetShareInfo(user.UserName, path.Substring(path.LastIndexOf("/") + 1));
                if (share != null)
                {
                    share.AbsPath = $"{Settings.Default.absRootPath}/{share.UserName}/{newName}";
                    manager.UpdateShareInfo(share);
                }
            }
            catch (Exception)
        ...
        public void UpdateFilesSize(User user, int fileCount, long fileSize, UIProcessBar bar, UILabel label)
        {
            try
            {
                //查询容量配置信息
                string cmd = $"echo '{fileCount} {fileSize}' > {Settings.Default.absRootPath}/{user.UserName}/.ftpquota";
                SshHelper.ExcuteCommand(cmd);
                if(user.QuotaSize == 0)
                {
                    bar.Visible = false;
                    label.Text = $"{FileHelper.UnitConversion(fileSize)}/无限制";
                }
                else
                {
                    bar.Value = (int)(fileSize / (user.QuotaSize * 1024 * 1024) * 100);
                    label.Text = $"{FileHelper.UnitConversion(fileSize)}/{FileHelper.UnitConversion(user.QuotaSize * 1024 * 1024)}";
                }           
            }
            catch (Exception)
            {
