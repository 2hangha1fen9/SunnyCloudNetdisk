    ...
    public partial class HomePanel : UIPage
    {
        ...
        private void Classfiy()
        {
            try
            {
                ...
                view.UpdateFilesSize(main.user, count, size, Capacity, CapacityInfo);
            }
            catch (Exception ex)
            {
                        ...
                        if (ShowAskDialog("提示", "确定要删除该文件吗?", UIStyle.Gray, false))
                        {
                            foreach (ListViewItem item in this.AllFile.SelectedItems)
                            {
                                ...
                                if (des.Type == FileType.Folder)//判断是否是文件夹,文件夹有自己的删除方法
                                {
                                    ...
                                    ShareDescirbe share = manager.GetShareInfo(main.user.UserName, (item.Tag as FileDescribe).Name);
                                    if (share != null)
                                    {
        ...
        private void fileOperator_Opening(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (uiTabControlMenu1.SelectedIndex != 0)
            {
                ...
                if (selected.SelectedItems.Count == 0)
                {
                    ...
                    this.fileOperator.Items["分享ToolStripMenuItem"].Visible = false;
                }
                else
                {
                    ...
                    this.fileOperator.Items["分享ToolStripMenuItem"].Visible = true;
                }
            }
            else
            {
        ...
        private void sharing_Click(object sender, EventArgs e)
        {
            ...
            if (uiTabControlMenu1.SelectedIndex != 0)
            {
                ...
                new ShareDialog(main.user, selected.SelectedItems).ShowDialog(this);
            }
            else
            {
                if (this.AllFile.SelectedItems.Count == 0)
                ...
                new ShareDialog(main.user, this.AllFile.SelectedItems).ShowDialog(this);
            }
        }
        private void 分享ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            sharing_Click(null, null);
        }
        #endregion              
    }
}
